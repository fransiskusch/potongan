...
export const AISettingsModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      setFeedback({ ok: true, msg: `Loaded ${result.models?.length ?? 0} models` });
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-labelledby="ai-settings-title">
      <div className="w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <label className="block space-y-1.5"><span className="text-sm font-medium text-neutral-300">Provider</span><select value={provider} onChange={(event) => setProvider(event.target.value as typeof provider)} className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-neutral-200 text-sm">{PROVIDERS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        ...
        <div className="p-4 border-t border-neutral-800 flex items-center justify-end gap-2">
          {provider !== "manual_ai" && <button type="button" onClick={handleFetch} disabled={loading || (isCustom && !customBaseUrl)} className="px-3.5 py-2 bg-neutral-800 hover:bg-neutral-700 disabled:opacity-40 text-neutral-200 text-xs font-medium rounded-xl flex items-center gap-1.5"><RefreshCw className="w-4 h-4" /> Fetch Models</button>}

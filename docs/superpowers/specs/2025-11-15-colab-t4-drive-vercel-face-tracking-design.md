...
| `web/` (rebrand UI) | Nama tampilan web UI berubah dari "Auto Clipper Cloud" menjadi **"Potongan.id"** (title browser `web/index.html`, header login `AuthGate.tsx`, header app & footer versi `App.tsx`, share-title default "Auto Clipper Video" di `HistoryList.tsx`/`StepResult.tsx`/`StepPrompt.tsx`, label "Initializing Auto Clipper..."). Hanya nama tampilan — domain tetap `clip.fransiskus.my.id`, env var & kode tidak berubah. Desktop app (Tauri, `src/`) tidak ikut di-rebrand. |
